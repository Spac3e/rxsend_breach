if SERVER then
	...
	net.Receive( "ScreengrabSendPart", function( _, ply )
		...
		if not ply.data then
			ply.data = {}
			ply.data[ 1 ] = data
			sendto:rtxappend( sg.blue, "收集第一部分" )
		else
			local num = table.getn( ply.data ) + 1
			ply.data[ num ] = data
			sendto:rtxappend( sg.blue, "收到 " .. num .. STNDRD( num ) .. " 部分" )
		end
		if table.getn( ply.data ) == ply.parts then
			ply.IsSending = nil
			sendto:rtxappend( sg.green, "准备发送数据 [" .. ply.parts .. " parts]" )
			local i = 1
			timer.Create( "SendDataBack", 0.1, ply.parts, function()
				...
				net.Send( sendto )
				sendto:rtxappend( sg.yellow, "发送 " .. i .. STNDRD( i ) .. " 部分" )
				i = i + 1
			end )
		end
	end )
...
if CLIENT then
    ...
    hook.Add("PostRender", "Screengrab", function()
        ...
        cl_rtxappend2( sg.green, "正在初始化", sgPly )        
        local function capture( q )
            ...
            cl_rtxappend2( color_white, "捕获 " .. len .. " 字节", sgPly )
            local parts = math.ceil( len / split )
            cl_rtxappend2( color_white, parts .. " 部分", sgPly )
            local partstab = {}
            for i = 1, parts do
                local min
                local max
            ...
            net.SendToServer()
            cl_rtxappend2( Color( 0, 255, 0 ), "准备发送数据", sgPly )
            local i = 1
            timer.Create( "ScreengrabSendParts", 0.1, amt, function()
                ...
                net.SendToServer()
                cl_rtxappend2( Color( 255, 255, 0 ), "发送 " .. i .. STNDRD( i ) .. " 部分", sgPly )
				net.Start( "Progress" )
