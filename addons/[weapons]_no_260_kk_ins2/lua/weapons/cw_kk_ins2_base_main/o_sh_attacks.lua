...
function SWEP:PrimaryAttack()
	...
	if IFTP then
		if self.projectileClass then
			if SERVER and ( self:GetClass() == "cw_kk_ins2_rpg" or self:GetClass() == "cw_kk_ins2_at4" ) then
				util.ScreenShake(self.Owner:GetPos(), 12.5, 15, 1.2, 200)
			end
			...
			if IsValid(self.Owner) and self.Owner.CommanderAbilityActive and self.Owner:IsPlayer() then
				timer.Remove("Goc_Commander_"..self.Owner:UniqueID())
				for _, wep in pairs(self.Owner:GetWeapons()) do wep:SetNoDraw(false) end
				self.Owner:SetNoDraw(false)
				self.Owner:RXSENDNotify("Из-за того что вы начали стрельбу невидимость была прекращена.")
				self.Owner.CommanderAbilityActive = nil
			end
			local multiplier = 1
