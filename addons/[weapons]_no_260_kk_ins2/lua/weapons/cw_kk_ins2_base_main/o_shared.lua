...
function SWEP:CycleFiremodes()
	...
	if self.FireMode != self.FireModes[t.last] and self.FireModes[t.last] then
		...
		if SERVER then
			self.Owner:RXSENDNotify("Вы выбрали режим стрельбы: "..self.FireModes[t.last])
		end
...
function SWEP:Reload()
	...
	if self.Owner:GetRoleName() == SCP912 and self:Clip1() > 0 then

		if SERVER and textcd <= SysTime() then
			textcd = SysTime() + 1
			self.Owner:BrTip(0, "[SCP-912]", Color(255,0,0), "Для перезарядки вы должны истратить всю обойму!", color_white)
		end
...
function SWEP:isNearWall()
	...
	--my no shoot in jump code modified by shaky

	if !self.Owner:IsOnGround() then
        
        self.Owner.OnTheGround = true
        lasttimefall = CurTime() + 0.1
        
        return true
    else
        if CurTime() >= lasttimefall then
            self.Owner.OnTheGround = false
            --return false
        else
            self.Owner.OnTheGround = true
            --return true
