...
SWEP.Slot = 1
SWEP.SlotPos = 0
SWEP.NormalHoldType = "gren"
SWEP.RunHoldType = "gren"
SWEP.FireModes = {"semi"}
...
//-----------------------------------------------------------------------------
// EquipAmmo replacement for SWEP.Primary.DefaultClip = 0 --
//-----------------------------------------------------------------------------
...
function SWEP:SetupDataTables()
	...
	self:NetworkVar("Bool", 4, "ReadyToThrow")
...
function SWEP:_attack(key)
	...
	elseif self.mustCook or (self.canCook and key == self:getControlls()) then 	// if wep allows it and pressed key is cooking key then cook
		if !self.KKINS2RCE then
			if SERVER then
				self.Owner:PlayGestureSequence("AHL_Attack_GREN_FRAG1", GESTURE_SLOT_ATTACK_AND_RELOAD, true)
			end
			timer.Simple(1, function()
				if IsValid(self) and IsValid(self.Owner) then
					self.dt.ReadyToThrow = true
				end
			end)
		end
