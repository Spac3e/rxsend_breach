...
if CLIENT then
	...
	function att:elementRender()
		...
		if element and IsValid(element.ent) then
			table.insert(beamAtts, element.ent:GetAttachment(1) || 0)
		end

		if element and element.models then
			for _,subElement in pairs(element.models) do
				if IsValid(subElement.ent) then
					table.insert(beamAtts, subElement.ent:GetAttachment(1) || 0)
				end
			end
		end

		if table.Count(beamAtts) < 1 then
			table.insert(beamAtts, self.CW_VM:GetAttachment(1) || 0)
		end
