...
function SWEP:ValidateAttachments()

	self.playSoundsOnInteract = false

	for name, status in pairs( self.ActiveAttachments ) do

		if ( !status ) then continue end

		for id, table in pairs( self.Attachments ) do
	...
	self.playSoundsOnInteract = true
		...
		if self.AttachmentModelsVM then
			...
			if att.isSight then
				...
				if self.SightWithRail then
					...
					if att.withoutRail or model.norail then
						state = false
					else
						state = true
					end
