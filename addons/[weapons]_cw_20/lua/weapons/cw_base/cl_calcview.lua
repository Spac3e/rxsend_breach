...
~ file: addons/all_weapons/lua/weapons/cw_base/cl_calcview.lua ~
...
function SWEP:CalcView(ply, pos, ang, fov)
	...
	if self.Owner then
		if self.ViewbobEnabled then
			...
			vel = 1--Length(GetVelocity(self.Owner))
...
function SWEP:stopHoldingBreath(time, regenTime, recoilMod)
	...
	if self.holdingBreath then
		...
		time = time || self.BreathDelay
		regenTime = regenTime || self.BreathRegenDelay
		...
		if ( self.Owner.Hold_Breath && self.Owner.Hold_Breath:IsPlaying() ) then

			self.Owner.Hold_Breath:Stop()

		end

		self.Owner.Hold_Breath = CreateSound( self.Owner, "nextoren/charactersounds/breathing/hold_breath/breath_out_" .. math.random( 1, 3 ) .. ".ogg" )
		self.Owner.Hold_Breath:SetSoundLevel( 120 )
		self.Owner.Hold_Breath:SetDSP( 17 )
		self.Owner.Hold_Breath:Play()
...
local spec_index, scp_index = TEAM_SPEC, TEAM_SCP
...
function SWEP.CreateMove(move)
	...
	if wep != NULL && wep.CW20Weapon then
		...
		if ( state == CW_CUSTOMIZE ) then
			...
			if ( isbool( ShouldFreeze ) ) then

				canAdjustAttachment = !shouldFreeze
			...
			if canAdjustAttachment then
				...
				if ( ply._holdAngles && !shouldFreeze ) then
		...
		elseif shouldFreeze == true then
			...
			ply._holdAngles = ply._holdAngles || ply:EyeAngles()
		...
		if ( wep.dt && aiming ) then

			if ( wep.Owner:KeyDown( IN_SPEED ) ) then

				if CT > wep.breathWait then

					if ( !wep.noBreathHoldingUntilKeyRelease && vel < wep.BreathHoldVelocityMinimum ) then

						if !( wep.holdingBreath && stamina > 20 ) then

							wep.holdingBreath = true
							wep.breathReleaseWait = CT + 0.5

							ply.Hold_Breath = CreateSound( ply, "nextoren/charactersounds/breathing/hold_breath/breath_hold_" .. math.random( 1, 3 ) .. ".ogg" )
							ply.Hold_Breath:SetSoundLevel( 120 )
							ply.Hold_Breath:SetDSP( 17 )
							ply.Hold_Breath:Play()
		...
		if wep.dt.BipodDeployed && wep.DeployAngle then
