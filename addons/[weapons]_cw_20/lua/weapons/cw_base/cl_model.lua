			...
			if muz2 then
				...
				if self.dt.Suppressed then
					if self.MuzzleEffectSupp then
						if not self.NoSilMuz then
							if (self.dt.State == CW_AIMING and self.SimulateCenterMuzzle) or (self.dt.State == CW_AIMING and self:canUseSimpleTelescopics()) then
								ParticleEffect(self.MuzzleEffectSupp, pos + self.Owner:GetVelocity() * 0.003 + EA:Forward() * 70 - EA:Up() * 3, EA, vm)
							else
								if self.PosBasedMuz then
									ParticleEffect(self.MuzzleEffectSupp, pos + self.Owner:GetVelocity() * 0.003, EA, vm) -- using velocity to add to the position 'simulates' attaching it to a control point
								else
					...
					if self.MuzzleEffect then
						particleEffect = self.dt.Suppressed and self.MuzzleEffectSupp or (self.MuzzleEffectWorld or self.MuzzleEffect)
						if CW_ALTERNATIVE_MUZZLEFLASH_3P[particleEffect] then
							particleEffect = CW_ALTERNATIVE_MUZZLEFLASH_3P[particleEffect]
						end
						local ammotype = game.GetAmmoName(self:GetPrimaryAmmoType())
						...
						if (self.dt.State == CW_AIMING and self.SimulateCenterMuzzle) or (self.dt.State == CW_AIMING and self:canUseSimpleTelescopics()) then
							ParticleEffect(particleEffect, pos + self.Owner:GetVelocity() * 0.003 + EA:Forward() * 70 - EA:Up() * 3, EA, vm)
						else
							if self.PosBasedMuz then
								ParticleEffect(particleEffect, pos + self.Owner:GetVelocity() * 0.003, EA, vm)
							else
								ParticleEffectAttach(particleEffect, PATTACH_POINT_FOLLOW, vm, muz)
							end
						end
					end
					...
					if particleEffect != "blue_weapon_muzzle_flash" then
						...
						dlight.Size = 150
						dlight.Decay = 4000
						dlight.DieTime = CurTime() + FrameTime()*12
					end
				end
			end
		end
...
function SWEP:applyOffsetToVM()
	...
	local pos = EyePos()
	local ang
	local z_offset = Vector(0, 0, -GetConVar("breach_config_cw_viewmodel_offset_z"):GetInt()/10)
	...
	if self.dt.State == CW_AIMING then
		offset_lerp = math.Approach(offset_lerp, 0, FrameTime()*3.6)
	else
		offset_lerp = math.Approach(offset_lerp, 1, FrameTime()*2.6)
	end
	...
	pos = pos+LerpVector( (offset_lerp), Vector(0,0,0), z_offset)
...
function SWEP:performViewmodelMovement()
	...
	elseif self.dt.State == CW_RUNNING or (((len > ws * self.RunStateVelocity and self.Owner:KeyDown(IN_SPEED)) or len > ws * 3 or (self.ForceRunStateVelocity and len > self.ForceRunStateVelocity)) and self.Owner:OnGround()) then
		...
		self.RunTime = self.RunTime + FT * (3.5 + math.Clamp(len / 120, 0, 5))
