...
function SWEP:DrawHUD()
	...
	if self.AimBreathingEnabled then
		...
		if self.Owner and self.Owner.Vignette_Action then
			surface.SetDrawColor(0, 0, 0, 255 * self.Owner.Vignette_Action)
			surface.SetTexture(Vignette)
			surface.DrawTexturedRect(0, 0, x, y)

			self.Owner.Vignette_Action = math.Clamp(self.Owner.Vignette_Action - 0.004, 0, 1)
		end
