...
function SWEP:Initialize()
	...
	if CLIENT then
		...
		self.AimBreathingEnabled_Orig = true
...
function SWEP:Deploy()
	...
	if ( !IsValid(self.Old_Owner) ) then

		self.Old_Owner = self.Owner

	end

	if ( self.Old_Owner != self.Owner ) then

		self:ValidateAttachments()
		self.Old_Owner = self.Owner
...
function SWEP:PrimaryAttack()
	...
	if IsFirstTimePredicted() then
		...
		local dlight = DynamicLight(self:EntIndex())
						...
						dlight.g = 255
						dlight.b = 255
						dlight.Brightness = 35
						dlight.Pos = pos + self.Owner:GetAimVector()
						dlight.Size = 150
						dlight.Decay = 1000
...
function SWEP:GetRecoilModifier(mod)
	...
	if self.Owner.recoilmultiplier then mod = mod * self.Owner.recoilmultiplier end
	return mod
end
...
function SWEP:DoImpactEffect(traceData, damageType)
	...
	if !traceData.HitSky then
		...
		effectdata:SetEntity(traceData.Entity)
		effectdata:SetOrigin( traceData.HitPos )
		effectdata:SetAngles( AngleRand() )

		if IsValid(traceData.Entity) and (traceData.Entity:IsPlayer() or traceData.Entity:GetClass() == "prop_ragdoll" ) then
			effectdata:SetColor(94, 84, 79)--effectdata:SetColor(175,0,0)
		end
		if CLIENT then
			util.Effect( "helicopter_impact", effectdata )
		end
	end
